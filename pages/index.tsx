...
export default function Home({ data }: { data: PageData }) {
  return (
    <div className="relative mx-auto w-[1240px]">
      {data?.hero && <Hero title={data?.hero?.title} image={data?.hero.image} />}
      
      {data?.blogPost && <BlogPost title={data?.blogPost?.title} content={data?.blogPost?.content} image={data?.blogPost.image} />}
